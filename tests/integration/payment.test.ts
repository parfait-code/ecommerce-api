...
describe("Payment Integration", () => {
  ...
  beforeAll(async () => {
    ...
    const catRes = await request(app)
      ...
      .send({
        ...
        slug: `payment-category-${timestamp}`,
      });
  ...
  afterAll(async () => {
    ...
    await prisma.product.deleteMany({
      where: { name: "Payment Test Product" },
    });
  ...
  describe("GET /payment-methods", () => {
    ...
    it("should include CASH_ON_DELIVERY as available", () => {
      return request(app)
        .get("/payment-methods")
        .then((res) => {
          const cod = res.body.data.find(
            (m: { id: string }) => m.id === "CASH_ON_DELIVERY",
          );
    ...
    it("should mark STRIPE, PAYPAL, CINETPAY as unavailable", async () => {
      const res = await request(app).get("/payment-methods");
      const unavailable = res.body.data.filter(
