...
describe('Checkout Integration', () => {
  ...
  afterAll(async () => {
    ...
    await prisma.product.deleteMany({ where: { name: 'Checkout Test Product' } })
  ...
  describe('POST /checkout', () => {
    ...
    it('should return 404 if basket not found', async () => {
      const res = await request(app)
        .post('/checkout')
        .set('Authorization', `Bearer ${userToken}`)
        .send({ basket_id: 'nonexistent-basket', shipping_address: shippingAddress })
  ...
  describe('POST /checkout/:checkout_id/complete', () => {
    ...
    it('should return 403 if user does not own the checkout', async () => {
      // Create another checkout owned by userToken
      ...
      // Try to complete with adminToken (different user)
      const res = await request(app)
