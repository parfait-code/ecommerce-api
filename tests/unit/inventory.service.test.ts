...
const mockInventoryRepository = inventoryRepository as jest.Mocked<typeof inventoryRepository>
...
describe('InventoryService', () => {
  describe('getAll', () => {
    ...
    it('should filter by category', async () => {
      ...
      expect(mockInventoryRepository.findAll).toHaveBeenCalledWith({ category: 'Electronics' })
    })
    ...
    it('should filter by location', async () => {
      ...
      expect(mockInventoryRepository.findAll).toHaveBeenCalledWith({ location: 'Yaoundé' })
    })
  })
  ...
  describe('getLowStock', () => {
    it('should return items below threshold', async () => {
      mockInventoryRepository.findLowStock.mockResolvedValue([mockInventoryItem])
  ...
  describe('create', () => {
    ...
    it('should throw 404 if product not found', async () => {
      ...
      await expect(
        inventoryService.create({ product_id: 999, warehouse_id: 'warehouse-cuid-1', quantity: 10 }),
    ...
    it('should throw 404 if warehouse not found', async () => {
      ...
      await expect(
        inventoryService.create({ product_id: 1, warehouse_id: 'nonexistent', quantity: 10 }),
    ...
    it('should throw 409 if inventory item already exists', async () => {
      mockProductRepository.findById.mockResolvedValue(mockProduct)
      mockWarehouseRepository.findById.mockResolvedValue(mockWarehouse)
      mockInventoryRepository.findByProductAndWarehouse.mockResolvedValue(mockInventoryItem)

      await expect(
        inventoryService.create({ product_id: 1, warehouse_id: 'warehouse-cuid-1', quantity: 10 }),
      ).rejects.toThrow(
        new AppError('Inventory item already exists for this product and warehouse', 409),
      )
    })
  })
  ...
  describe('update', () => {
    it('should update inventory item', async () => {
      ...
      const result = await inventoryService.update('inventory-cuid-1', { quantity: 100 })
  ...
  describe('delete', () => {
    it('should delete inventory item and return message', async () => {
      ...
      expect(mockInventoryRepository.delete).toHaveBeenCalledWith('inventory-cuid-1')
  ...
  describe('transfer', () => {
    ...
    it('should transfer stock between warehouses (destination exists)', async () => {
      ...
      const destinationItem = { ...mockInventoryItem, id: 'inventory-cuid-2', warehouseId: 'warehouse-cuid-2', quantity: 20 }
      ...
      mockInventoryRepository.decrementQuantity.mockResolvedValue({ ...sourceItem, quantity: 70 })
      mockInventoryRepository.incrementQuantity.mockResolvedValue({ ...destinationItem, quantity: 50 })
      ...
      expect(mockInventoryRepository.decrementQuantity).toHaveBeenCalledWith('inventory-cuid-1', 30)
      expect(mockInventoryRepository.incrementQuantity).toHaveBeenCalledWith('inventory-cuid-2', 30)
    ...
    it('should throw 400 if insufficient stock', async () => {
      ...
      mockInventoryRepository.findById.mockResolvedValue(sourceItem)
      mockInventoryRepository.findByProductAndWarehouse.mockResolvedValue(sourceItem)
      ...
      ).rejects.toThrow(new AppError('Insufficient stock in source warehouse', 400))
    })
  })
})