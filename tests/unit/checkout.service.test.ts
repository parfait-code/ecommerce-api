...
const mockCheckoutRepository = checkoutRepository as jest.Mocked<typeof checkoutRepository>
...
const mockCheckout = {
  ...
  total: 150.00,
  items: [{ productId: 1, name: 'Test Product', price: 50.00, quantity: 3 }],
  orderId: null,
  createdAt: new Date(),
  updatedAt: new Date(),
...
describe('CheckoutService', () => {
  describe('create', () => {
    it('should create a checkout from a basket', async () => {
      ...
      expect(mockBasketRepository.findById).toHaveBeenCalledWith('basket-cuid-1')
      expect(mockCheckoutRepository.create).toHaveBeenCalledWith(
        1,
        mockCreateDto,
        150.00,
        [{ productId: 1, name: 'Test Product', price: 50.00, quantity: 3 }],
      )
  ...
  describe('complete', () => {
    it('should complete checkout and create an order', async () => {
      const completedCheckout = { ...mockCheckout, status: 'COMPLETED', orderId: 'order-cuid-1' }
      mockCheckoutRepository.findById.mockResolvedValue(mockCheckout)
      ...
      expect(mockCheckoutRepository.complete).toHaveBeenCalledWith('checkout-cuid-1', 'order-cuid-1')
    ...
    it('should throw 403 if user does not own the checkout', async () => {
      ...
      await expect(checkoutService.complete('checkout-cuid-1', 99)).rejects.toThrow(
    ...
    it('should throw 400 if checkout already completed', async () => {
      ...
      await expect(checkoutService.complete('checkout-cuid-1', 1)).rejects.toThrow(
        new AppError('Checkout already completed', 400),
      )
    })
  })
})