...
describe('BasketService', () => {
  ...
  describe('addProduct', () => {
    it('should add a product to the basket', async () => {
      ...
      expect(mockBasketRepository.addItem).toHaveBeenCalledWith('basket-cuid-1', 1, 2)
    ...
    it('should throw 404 if product not found', async () => {
      ...
      await expect(
        basketService.addProduct('basket-cuid-1', { product_id: 999, quantity: 1 }),
  ...
  describe('updateQuantity', () => {
    it('should update the quantity of an existing item', async () => {
      mockBasketRepository.findById
        ...
        .mockResolvedValueOnce({ ...mockBasketWithItem, items: [{ ...mockBasketWithItem.items[0], quantity: 5 }] })
      ...
      expect(mockBasketRepository.updateQuantity).toHaveBeenCalledWith('basket-cuid-1', 1, 5)
    ...
    it('should throw 404 if basket not found', async () => {
      ...
      await expect(
        basketService.updateQuantity('nonexistent', { product_id: 1, quantity: 5 }),
      ).rejects.toThrow(new AppError('Basket not found', 404))
    })
    ...
    it('should throw 404 if product not in basket', async () => {
      ...
      await expect(
        basketService.updateQuantity('basket-cuid-1', { product_id: 999, quantity: 5 }),
  ...
  describe('removeProduct', () => {
    it('should remove a product from the basket', async () => {
      ...
      const result = await basketService.removeProduct('basket-cuid-1', { product_id: 1 })

      expect(mockBasketRepository.removeItem).toHaveBeenCalledWith('basket-cuid-1', 1)
      expect(result).toEqual(mockBasket)
    })
